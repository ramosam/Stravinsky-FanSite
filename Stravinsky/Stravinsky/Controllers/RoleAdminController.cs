...
= await roleManager.CreateAsync(new IdentityRole(name)); if (result.Succeeded)
        ...
        {
            IdentityRole role = await roleManager.FindByIdAsync(id); if (role != null)
            {
        ...
        {
            IdentityRole role = await roleManager.FindByIdAsync(id); List<AppUser> members = new List<AppUser>(); List<AppUser> nonMembers = new List<AppUser>();
        ...
        public async Task<IActionResult> Edit(RoleModificationModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                {
                    AppUser user = await userManager.FindByIdAsync(userId); if (user != null)
                    {
